...
/** Compara dos hashes en tiempo constante, sin filtrar información por el tiempo. */
export function sonElMismoToken(unHash: string, otroHash: string): boolean {
  const uno = Buffer.from(unHash, 'hex');
  const otro = Buffer.from(otroHash, 'hex');

  if (uno.length !== otro.length || uno.length === 0) {
    return false;
  }
